...
CAtmoDuinoConnection::CAtmoDuinoConnection(CAtmoConfig *cfg) : CAtmoConnection(cfg) {
...
CAtmoDuinoConnection::~CAtmoDuinoConnection() {
}

ATMO_BOOL CAtmoDuinoConnection::OpenConnection() {
#if defined(_ATMO_VLC_PLUGIN_)
...
void CAtmoDuinoConnection::CloseConnection() {
    if (m_hComport != INVALID_HANDLE_VALUE) {
#if defined(WIN32)
...
ATMO_BOOL CAtmoDuinoConnection::isOpen(void) {
    return (m_hComport != INVALID_HANDLE_VALUE);
}

ATMO_BOOL CAtmoDuinoConnection::HardwareWhiteAdjust(int global_gamma,
    int global_contrast,
    int contrast_red,
    int contrast_green,
    int contrast_blue,
...
ATMO_BOOL CAtmoDuinoConnection::SendData(pColorPacket data) {
    if (m_hComport == INVALID_HANDLE_VALUE)
        return ATMO_FALSE;
...
ATMO_BOOL CAtmoDuinoConnection::CreateDefaultMapping(CAtmoChannelAssignment *ca)
{
...
ATMO_BOOL CAtmoDuinoConnection::setChannelColor(int channel, tRGBColor color)
{
...
ATMO_BOOL CAtmoDuinoConnection::setChannelValues(int numValues, unsigned char *channel_values)
{
    DWORD iBytesWritten;
...
int CAtmoDuinoConnection::getNumChannels()
{
    return m_pAtmoConfig->getAtmoClLeds();
}

int CAtmoDuinoConnection::getBufferSize(){
    return getNumChannels() * 3 + 4;
}

char *CAtmoDuinoConnection::getChannelName(int ch)
{
    if (ch < 0) return NULL;
    char buf[30];
...
ATMO_BOOL CAtmoDuinoConnection::ShowConfigDialog(HINSTANCE hInst, HWND parent, CAtmoConfig *cfg)
{
    CAtmoDuinoConfigDialog *dlg = new CAtmoDuinoConfigDialog(hInst, parent, cfg);
